{
    ...
    auto fname = [INCLUDE_TAU](const string& category) {
        stringstream res;
        if(INCLUDE_TAU)
    ...
    ScanChain(ch_ft, &out_ft, INCLUDE_TAU);
    out_ft.Close()
    ...
    ScanChain(ch_ttw, &out_ttw, INCLUDE_TAU);
    out_ttw.Close()
    ...
    ScanChain(ch_ttz, &out_ttz, INCLUDE_TAU);
    out_ttz.Close()
    ...
    ScanChain(ch_tth, &out_tth, INCLUDE_TAU);
    out_tth.Close()
}
