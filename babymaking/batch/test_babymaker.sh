...
# NOTE dis2 (based on python flask) will choke when there's 4-5 concurrent SNT requests
# so don't run these in parallel. Either need to fix the dis2 flask service or turn `threaded=False`
