...
int ScanChain( TChain* chain, TString outfile, TString option="", bool fast = true, int nEvents = -1) {//, string skimFilePrefix = "test") {
  ...
  bool do_jet_cut = true; // FIXME note
  ...
  // float sf_HLT_Ele12_CaloIdL_TrackIdL_IsoVL_PFJet30 = 1.8080; // unused
  // float sf_HLT_Ele17_CaloIdM_TrackIdM_PFJet30 = 2.0126;
  // float sf_HLT_Ele23_CaloIdL_TrackIdL_IsoVL_PFJet30 = 2.0191;
  // float sf_HLT_Ele23_CaloIdM_TrackIdM_PFJet30 = 2.0149; // unused
  // float sf_HLT_Ele8_CaloIdM_TrackIdM_PFJet30 = 1.6620;
  // float sf_HLT_Ele8_CaloIdL_TrackIdL_IsoVL_PFJet30 = 1.6531;
  // float sf_HLT_Mu17_TrkIsoVVL = 0.9206;
  // float sf_HLT_Mu17 = 0.9299;
  // float sf_HLT_Mu8_TrkIsoVVL = 0.3560;
  // float sf_HLT_Mu8 = 0.5758;

  // float sf_HLT_Ele17_CaloIdM_TrackIdM_PFJet30 = 4.3282;
  // float sf_HLT_Ele23_CaloIdL_TrackIdL_IsoVL_PFJet30 = 4.3375;
  // float sf_HLT_Ele8_CaloIdM_TrackIdM_PFJet30 = 3.5211;
  // float sf_HLT_Ele8_CaloIdL_TrackIdL_IsoVL_PFJet30 = 3.5363;
  // float sf_HLT_Mu17_TrkIsoVVL = 2.4255;
  // float sf_HLT_Mu17 = 2.4247;
  // float sf_HLT_Mu8_TrkIsoVVL = 1.6670;
  // float sf_HLT_Mu8 = 1.6800;

  // // v3 needs hacky ZSF
  // float sf_HLT_Ele17_CaloIdM_TrackIdM_PFJet30 = 1.8072;
  // float sf_HLT_Ele23_CaloIdL_TrackIdL_IsoVL_PFJet30 = 1.8111;
  // float sf_HLT_Ele8_CaloIdM_TrackIdM_PFJet30 = 1.4702;
  // float sf_HLT_Ele8_CaloIdL_TrackIdL_IsoVL_PFJet30 = 1.4765;
  // float sf_HLT_Mu17_TrkIsoVVL = 1.0127;
  // float sf_HLT_Mu17 = 1.0124;
  // float sf_HLT_Mu8_TrkIsoVVL = 0.6960;
  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) {
	...
	if (!isDoubleMuon && abs(id())!=11) continue;
      ...
      float puw = 1; // getTruePUw_Moriond(nvtx());
      if (!isDataFromFileName) puw = puweight();
      // float puw = 1.0;
      float weight = scale1fb()*lumi*puw;

      // // FIXME eventually. The DY sample had xsec too large.
      // if (isDY) weight /= 2.395;
      ...
      // NOTE. throw away low uncor pt if lower than trigger thresholds
          if (abs(id())==13) {
	...
	if(jets_recorr[i].pt() > 40. && fabs(jets_recorr[i].eta()) < 2.4) {
      ...
      if (noMissHits) {
          if (abs(id())==11 && exp_innerlayers() > 0) continue;
      }
