    ...
    for proc in d_hists["shapes_prefit"].values()[0].keys():
        ...
        for region in d_hists["shapes_fit_s"].keys():
            ...
            if postfit is not None: 
                ipostfit += postfit.get_integral()
                err += postfit.get_integral_and_error()[1]
            if prefit is not None: iprefit += prefit.get_integral()
        d_sfs[proc] = ipostfit/iprefit
        d_postfit_errs[proc] = err/ipostfit
...
def make_plots(
    ...
    for region in regions:
        ...
        # mpl_legend_params["framealpha"] = 0.8
        ...
        def ax_main_callback(ax,title=title):
            ...
            if region.lower() == "srdisc":
                ax.set_ylim([0.08,ax.get_ylim()[1]*(5.2+5.0)])
            else:
        ...
        print "sfs",d_sfs
        print "totpostfitrelerrs",d_postfit_errs
...
def print_table(d_yields, regions="srcr",precision=2,blinded=False,paper=False):
    ...
    if paper:
        ...
        colnames = ["","SM background","$\\tttt$","Total","Data"]
        procs = ["total_background","tttt","total","data"]
    if regions == "srcr":
