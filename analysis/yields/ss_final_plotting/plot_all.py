...
def worker(info):
    ...
    for region in regions:
        ...
        if var in ["ht","met","njets","nbtags","mtmin"]:
            if var in ["ht","met"]:
        ...
        if (var.lower() in ["charge"]):
            xticks = ["$(-)$","$(+)$",""]
            mpl_xtick_params = dict(rotation=0, fontsize=14)

        if analysis == "ss" and region in ["SRLL","SRLM"]:
            mpl_xtick_params = dict(fontsize=11)
        ...
        if var in ["ht","met","mtmin","njets","nbtags","charge"]:
        ...
        if var in ["charge"] and region in ["br"]:
            ...
            def extend(x):
                x._edges[-1] += 1.
            print(data._edges,data._counts)
            map(extend,bgs+[data]+sigs)
            print(data._edges,data._counts)
