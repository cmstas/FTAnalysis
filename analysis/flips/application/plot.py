...
from common.matplottery.plotter import plot_stack
from common.matplottery.utils import Hist1D, MET_LATEX
...
def make_plots(outputdir="plots", inputfile="outputs/histos.root", prefix=""):
    ...
    for var, xlabel in [
        ...
        obs = Hist1D(f["clos_{}_plot_data".format(var)], label="obs (data)")
        pred = Hist1D(f["clos_{}_plot_MC".format(var)], label="pred (data)", color=(28/255.,168/255.,19/255.))
        obsMC = Hist1D(f["clos_{}_plot_MCp".format(var)], label="obs (MC)", color=(68/255.,61/255.,165/255.))
        pred *= sf
        ratio = obs/pred
        fname = "{}/{}{}.png".format(outputdir,prefix,var)
