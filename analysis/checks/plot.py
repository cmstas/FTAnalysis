...
def cutoff(hin, binnum):
    hout = r.TH1F(hin.GetName(), hin.GetTitle(), binnum,0,binnum)
    return hout
    return hin



def make_eff_plot(hnum,hden,title="Title",xtitle="x",fname="plots/plot.pdf",cutoffat8=False):
    c1 = r.TCanvas()
    hnum.Sumw2()
    hden.Sumw2()
    if cutoffat8:
        hnum = cutoff(hnum,binnum=8)
        hden = cutoff(hden,binnum=8)
...
def main():
    ...
    make_eff_plot(hnum=fiso.Get("h_num_vs_nj"),hden=fiso.Get("h_den_vs_nj"), title="Lepton iso. eff. for #geq 2L tttt events", xtitle="Njets", fname="plots/isoeff_nj.pdf") #, cutoffat8=True)
    make_eff_plot(hnum=fiso.Get("h_num_vs_nj2"),hden=fiso.Get("h_den_vs_nj2"), title="Lepton iso. eff. for 2L tttt events", xtitle="Njets", fname="plots/isoeff_nj2.pdf") #, cutoffat8=True)
    make_eff_plot(hnum=fiso.Get("h_num_vs_nj3"),hden=fiso.Get("h_den_vs_nj3"), title="Lepton iso. eff. for #geq 3L tttt events", xtitle="Njets", fname="plots/isoeff_nj3.pdf") #, cutoffat8=True)

    set_palette()
    make_2d_plot(hist=fbtag.Get("h_dr_vs_pt"), title="Max #DeltaR(b-tag, lepton) vs lepton p_{T}", xtitle="Lepton p_{T}", ytitle="Max #Delta R", fname="plots/dr_vs_pt.pdf")
    make_2d_plot(hist=fbtag.Get("h_mindr_vs_pt"), title="Min #DeltaR(b-tag, lepton) vs lepton p_{T}", xtitle="Lepton p_{T}", ytitle="Min #DeltaR", fname="plots/mindr_vs_pt.pdf")

    os.system("niceplots plots plots_checks_Jul10")
